...
def determine_ace_value(total, ace)
  total + DECK_VALUES[ace] <= GAME_VALUE ? 11 : 1
end
...
loop do
  ...
  loop do
    ...
    if user_hand_total <= GAME_VALUE
      ...
      prompt("BUST! You went over #{GAME_VALUE}!")
      bust = true
      dealer_score += 1
      break
    end
  ...
  loop do
    ...
    if dealer_hand_total >= DEALER_VALUE && dealer_hand_total <= GAME_VALUE
      prompt('The dealer stays. Time to reveal who won.')
      break
    elsif dealer_hand_total > GAME_VALUE
      bust = true
      player_score += 1
